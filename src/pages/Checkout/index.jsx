...
import { getCartSession } from "../../../../store/cart/cartAction";
...
export default function Checkout() {
  ...
  const payMethods = [
    { name: "Cartão de Credito", slug: "credit_card" },
    { name: "Boleto", slug: "boleto" },
  ];
  useEffect(() => {
    Cart.show(cartSession.uid).then((response) => {

      setCheckoutData(response.data.cart_data);
      ...
      pagarme.client
        ...
        .then((client) =>
          client.transactions.calculateInstallmentsAmount({
            ...
            free_installments: 3,
            interest_rate: 10,
            amount: parseInt(response.data.cart_data.totalAmount * 100),
          })
        )
        .then((response, key) => {
          let items = [];
  ...
  const handleSubmitPayment = (e) => {
    ...
    Payment.create(checkoutData).then((response) =>
      console.log(response, "irraaa")
    );
  };
  return (
    <div
                  ...
                  <div className={"form-row"}>
                    <div className={"form-group col-md-4"}>
                      ...
                      <input
                        ...
                        className={"form-control"}
                        onChange={handleCustomer}
                        defaultValue={customerData.address.postcode}
                      />
                    </div>
                    <div className={"form-group col-md-8"}>
                      ...
                      <input
                        ...
                        className={"form-control"}
                        onChange={handleCustomer}
                        defaultValue={customerData.address.address_1}
                      />
                    </div>
                    <div className={"form-group col-md-6"}>
                      <label>Bairro</label>
                      <input
                        ...
                        className={"form-control"}
                        onChange={handleCustomer}
                        defaultValue={customerData.address.address_2}
                      />
                    </div>
                    <div className={"form-group col-md-6"}>
                      <label>Cidade</label>
                  ...
                  <div className={`${css.payMentMethod} form-row`} key={key}>
                    <div className={`${css.payMentMethodHead} form-check`}>
                      <input
                        ...
                        value={method.slug}
                      />
                      <label className="form-check-label" htmlFor={method.slug}>
                        {" "}
                                ...
                                {installments.map((item, key) => (
                                  ...
                                  >
                                    {`${
                                      ...
                                      (item.installment_amount / 100).toFixed(2)
                                    )}`}
                                  </option>
                                ))}
                              </select>
