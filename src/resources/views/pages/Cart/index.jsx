...
function Checkout({ plans, ...props }) {
  ...
  useEffect(() => {
    Cart.show(cartSession.uid).then((response) => {
      console.log(response.data.cart_data, "mano q merdaaaa!!");
      setPassagers(response.data.cart_data["passagers"]);
      setMyCart(response.data.cart_data);
      TripSchedule.additionalPackages(
        response.data.cart_data.code
      ).then((response) => setadditionalPackages(response.data));
      calculateTotalPassagerAmount(response.data.cart_data);
    });
  }, []);
